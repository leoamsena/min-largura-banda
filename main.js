const initialSolution = (lista_ajacencia) => {
  ...
  no_visitado = no_visitado.fill(false);
  ...
  while (label < len_graph + 1) {
    ...
    for (let aux = 0; aux < len_graph; aux += 1) {
      if (sequenica_nos_visitados[aux] !== undefined)
        newVet[aux] = sequenica_nos_visitados[aux];
      if (s[aux] !== undefined) newVet[aux] = s[aux];
      if (sequenica_nos_visitados[aux] === undefined && s[aux] === undefined)
        newVet[aux] = undefined;
    }
