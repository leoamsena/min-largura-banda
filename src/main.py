from ReducaoLarguraBanda import ReducaoLarguraBanda
import time
...
"""
name = "../downloadMatrix/HB/1138_bus"
arq_name = name[name.rindex("/"):]
rlb = ReducaoLarguraBanda(name)
rlb.salvarImagemAntes(arq_name)
rcm = rlb.reverseCuthillMckee()
rlb.salvarImagem(rcm, arq_name, "after")
"""
for name in names:
    try:
        print("name = ", name)
        ...
        rlb = ReducaoLarguraBanda(
            "/home/leoamsena/UFLA-2020-1/CPA/trab 4/downloadMatrix/"+name)
        rlb.salvarImagemAntes(arq_name)
        tempo_inicial = time.time()
        print("Largura de banda incial:", rlb.calcularLarguraBandaInicial())
        rcm = rlb.reverseCuthillMckee()
        tempo_final = time.time()
        print("tempo = ", tempo_final - tempo_inicial)
        rlb.salvarImagem(rcm, arq_name, "after")
        print("Largura de banda: ", rlb.calcularLarguraBanda(rcm))
        a = input()
